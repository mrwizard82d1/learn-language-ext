...
- **Solution format: classic `.sln` (not `.slnx`).** .NET 10's `dotnet new sln` defaults to `.slnx`; regenerated as classic `.sln` via `--format sln`. Rationale: removes a Gateway/WSL variable for a learning project (CLI build/test is identical either way). Research found NO documented issue tying `.slnx` load failures to Gateway+WSL on Rider 2026.1 — the downgrade is precautionary, not a fix for a confirmed bug. (Note for future: there's a *deterministic* Linux path-casing `.slnx` bug, RIDER-135025, if we ever revisit `.slnx` — keep `Path=` casing matching on-disk dirs.)
